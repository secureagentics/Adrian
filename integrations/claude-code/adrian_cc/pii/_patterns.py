...
Vendored verbatim from ``sdk/python/adrian/pii/_patterns.py``.  The
plugin ships its own dependencies so it can run without installing
the SDK, and this module is stdlib-only, so it is copied rather than
imported.  Keep the two in sync: ``tests/test_pii.py`` fails if they
drift while the SDK source is present.
"""
